...
from llmtuner.dsets.add_demon import add_random_demon, add_Sim_CSE_demon, add_bert_demon, add_random_paragraph
...
def preprocess_dataset(
    ...
    with training_args.main_process_first(desc="dataset map pre-processing"):
        if (training_args.do_predict):
            ...
            print(dataset[0])
            print(len(dataset))
            if data_args.demon_method == "random":
                dataset = add_random_demon(data_args.attack, dataset, data_args.shots_num)
            elif data_args.demon_method == "SimCSE":
                dataset = add_Sim_CSE_demon(data_args.attack, dataset, data_args.shots_num)
            # elif data_args.demon_method == "bert":
            #     dataset = add_bert_demon(dataset, data_args.shots_num)
            elif data_args.demon_method == "ran_par":
                dataset = add_random_paragraph(dataset, data_args.shots_num)
            print(type(dataset[0]))
            print(dataset[0])
            print(len(dataset))
